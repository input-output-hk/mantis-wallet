...
const configGetter = convict({
  networkName: {
    ...
    doc: 'Which network Mantis Wallet should connect to',
  },
  rpcAddress: {
    default: 'https://127.0.0.1:8546/',
    format: 'own-url',
    arg: 'rpc-address',
    env: 'MANTIS_WALLET_RPC_ADDRESS',
  ...
  mantis: mantisConfig({
    ...
    additionalSettings: {
      ...
      'mantis.async.ask-timeout': '59.seconds',
    },
  }),
})
...
export const config = loadConfigs([
  {
    ...
    path: path.resolve(homedir(), 'mantis-wallet-config.json5'),
  },
  {
  ...
  {
    name: 'environment variable MANTIS_WALLET_CONFIG_FILE',
    path: process.env.MANTIS_WALLET_CONFIG_FILE || '',
  },
])
