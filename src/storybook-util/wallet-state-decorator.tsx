...
import {StoreWalletData, WalletState, WalletStateParams, WalletStatus} from '../common/wallet-state'
import {createInMemoryStore} from '../common/store'
import {defaultWeb3} from '../web3'

const web3 = defaultWeb3()

const defaultMockWalletData: StoreWalletData['wallet'] = {
  txHistory: {},
  addressBook: {
...
export interface WithWalletStateParameters {
  walletData: Partial<StoreWalletData['wallet']>
  extraWalletStateParams: Partial<WalletStateParams>
}

const WithWalletState: StoryWrapper = (
  storyFn: StoryGetter,
  context: StoryContext,
  {parameters}: {parameters: Partial<WithWalletStateParameters>},
): JSX.Element => {
  const content = storyFn(context)

  const walletData = {wallet: {...defaultMockWalletData, ...parameters?.walletData}}
  const extraWalletStateParams = {...parameters?.extraWalletStateParams}

  const initialState = {
    walletStatus: 'LOADED' as WalletStatus,
    web3,
    isMocked: true,
    store: createInMemoryStore(walletData),
    ...extraWalletStateParams,
  }
