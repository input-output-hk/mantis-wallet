...
describe('TLS', () => {
  const keyStorePath = path.resolve(__dirname, './data/mantisCA.p12')
  const passwordPath = path.resolve(__dirname, './data/password')
  const pkcsRawData = fs.readFileSync(path.resolve(__dirname, './data/mantisCA.p12'))
  const pemRawData = fs.readFileSync(path.resolve(__dirname, './data/mantisCA.pem'), {
    encoding: 'utf8',
  })
