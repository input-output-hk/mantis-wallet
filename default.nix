...
pkgs.stdenv.mkDerivation {
  ...
  MANTIS_DIST_DIR = mantis-dist + "/mantis-dist";
  ...
  installPhase = {
    x86_64-darwin = ''
      ...
      cp -r dist/Mantis-Wallet-darwin-x64/Mantis-Wallet.app $_
    '';
    x86_64-linux = ''
      yarn --offline run build-all
      mkdir -p $out/{bin,share/mantis-wallet}
      mv build/main/package.json .
      mv build package.json $out/share/mantis-wallet/
      ln -s $MANTIS_DIST_DIR $out/share/mantis-dist
      chmod 0755 $out/share/ -R
      echo "#!${pkgs.runtimeShell}" > $out/bin/mantis-wallet
      echo "cd $out/share/mantis-wallet" >> $out/bin/mantis-wallet
      echo "${pkgs.electron_8}/bin/electron $out/share/mantis-wallet/build/main/main.js" >> $out/bin/mantis-wallet
      chmod 0755 $out/bin/mantis-wallet
      wrapProgram $out/bin/mantis-wallet --set MANTIS_PACKAGE_DIRECTORY "../mantis-dist/mantis"
    '';
  }.${system};
}
